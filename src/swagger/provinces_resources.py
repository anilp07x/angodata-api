...
class ProvinceListResource(Resource):
    ...
    def get(self):
        ...
        if use_pagination and use_database and hasattr(ProvinceService, "get_all_paginated"):
            ...
            provinces = ProvinceService.get_all()
            return {"success": True, "total": len(provinces), "data": provinces}, 200
...
class ProvinceResource(Resource):
    ...
    def get(self, id):
        ...
        if not province:
            return {"success": False, "message": f"Província com ID {id} não encontrada"}, 404

        return {"success": True, "data": province}, 200
    ...
    def put(self, id):
        ...
        try:
            ...
            return {"success": False, "message": "Dados inválidos", "errors": err.messages}, 400
        ...
        return {"success": True, "message": "Província atualizada com sucesso", "data": updated}, 200
    ...
    def delete(self, id):
        ...
        if ProvinceService.delete(id):
            return {"success": True, "message": f"Província ID {id} deletada com sucesso"}, 200
...
class ProvinceCreateResource(Resource):
    ...
    def post(self):
        ...
        return {"success": True, "message": "Província criada com sucesso", "data": new_province}, 201
...
class ProvinceBulkCreateResource(Resource):
    ...
    def post(self):
        ...
        if not data or "provinces" not in data:
            return {"success": False, "message": "Formato inválido. Esperado: {provinces: [...]}"}, 400

        if not hasattr(ProvinceService, "bulk_create"):
            return {"success": False, "message": "Operação bulk não disponível no modo atual"}, 400

        result = ProvinceService.bulk_create(data["provinces"])

        return {"success": True, "message": f"{result['created']} províncias criadas com sucesso", **result}, 201
