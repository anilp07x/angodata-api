...
from src.schemas.user_schema import UserLoginSchema, UserRegistrationSchema, UserResponseSchema
...
def register():
    ...
    try:
        ...
        if new_user:
            return jsonify({"success": True, "message": "Usuário registrado com sucesso", "data": new_user}), 201
        else:
            ...
            if AuthService.get_user_by_email(data["email"]):
                ...
                return jsonify({"success": False, "message": "Erro ao registrar usuário"}), 400
    ...
    try:
        ...
        if user:
            ...
            access_token = create_access_token(
                ...
                additional_claims={"email": user["email"], "username": user["username"], "role": user["role"]},
            )
            ...
            AuditLogger.log_action(action="LOGIN_FAILED", resource_type="user", details={"email": data.get("email")})
...
def get_all_users():
    ...
    try:
        ...
        if claims.get("role") != "admin":
            return jsonify({"success": False, "message": "Acesso negado. Permissões insuficientes"}), 403

        users = AuthService.get_all_users()

        return jsonify({"success": True, "total": len(users), "data": users}), 200
...
def get_audit_logs():
    ...
    try:
        ...
        # Buscar logs
        logs = AuditLogger.get_logs(limit=limit, action=action, resource_type=resource_type, user_id=user_id)

        return jsonify({"success": True, "total": len(logs), "data": logs}), 200
    except Exception as e:
        return jsonify({"success": False, "message": f"Erro ao buscar logs de auditoria: {str(e)}"}), 500
