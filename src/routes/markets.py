...
def get_all_markets():
    ...
    return jsonify({"success": True, "total": len(markets), "data": markets}), 200
...
def get_market_by_id(market_id):
    ...
    if market:
        return jsonify({"success": True, "data": market}), 200
    else:
        return jsonify({"success": False, "message": f"Mercado com ID {market_id} não encontrado"}), 404
...
def create_market():
    ...
    try:
        ...
        if new_market:
            return jsonify({"success": True, "message": "Mercado criado com sucesso", "data": new_market}), 201
        else:
        ...
        return jsonify({"success": False, "message": f"Erro ao criar mercado: {str(e)}"}), 500
...
def update_market(market_id):
    ...
    try:
        ...
        if updated_market:
            return jsonify({"success": True, "message": "Mercado atualizado com sucesso", "data": updated_market}), 200
        else:
            return (
                jsonify(
                    {"success": False, "message": "Mercado não encontrado ou município não pertence à província especificada"}
                ),
                404,
            )
        ...
        return jsonify({"success": False, "message": f"Erro ao atualizar mercado: {str(e)}"}), 500
...
def delete_market(market_id):
    ...
    try:
        ...
        if deleted:
            return jsonify({"success": True, "message": "Mercado deletado com sucesso"}), 200
        else:
            return jsonify({"success": False, "message": f"Mercado com ID {market_id} não encontrado"}), 404

    except Exception as e:
        return jsonify({"success": False, "message": f"Erro ao deletar mercado: {str(e)}"}), 500
