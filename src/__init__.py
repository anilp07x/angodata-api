...
def init_database_if_enabled(app):
    ...
    if use_database:
        from src.database.base import close_database, init_database
...
def load_persisted_data():
    ...
    from src.models import hospital, market, municipality, province, school, user
...
def register_blueprints(app):
    ...
    Args:
        app (Flask): Instância da aplicação Flask
    """
    from src.routes import auth_bp, hospitals_bp, markets_bp, municipalities_bp, provinces_bp, schools_bp
...
def configure_jwt_handlers(jwt):
    ...
    def expired_token_callback(jwt_header, jwt_payload):
        ...
        return jsonify({"success": False, "message": "O token de autenticação expirou. Por favor, faça login novamente."}), 401

    @jwt.invalid_token_loader
    def invalid_token_callback(error):
        """Manipulador para token inválido."""
        return (
            jsonify({"success": False, "message": "Token de autenticação inválido. Por favor, forneça um token válido."}),
            422,
        )
    ...
    def unauthorized_callback(error):
        ...
        return jsonify({"success": False, "message": "Token de autenticação não fornecido. Por favor, faça login."}), 401
    ...
    def revoked_token_callback(jwt_header, jwt_payload):
        ...
        return (
            jsonify({"success": False, "message": "O token de autenticação foi revogado. Por favor, faça login novamente."}),
            401,
        )
